...
import {
  Instagram, Send, Share2, Target, Globe,
  Heart, MessageCircle, Users, RefreshCw,
  Trophy, Play, Eye
...
// Clave de persistencia: el panel vive en su propia tab, así que guardamos el
// avance en localStorage para que (a) no se pierda al cambiar de tab y
// (b) DashboardView pueda prellenar el modal de cierre de turno.
const STORAGE_KEY = 'essenza_marketing_panel'
...
function persist() {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(report.value)) } catch { }
}

function restore(): any | null {
  try {
...
const fetchSocialAccounts = async () => {
  ...
  try {
    ...
    report.value.socialActivity = res.data.map((acc: any) => {
      // Si había avance guardado para esta cuenta, lo restauramos
      const prev = saved?.socialActivity?.find((a: any) => a.accountId === acc.id)
      return prev || {
        accountId: acc.id,
        handle: acc.handle,
    ...
    if (saved) {
      report.value.reelsEdited = saved.reelsEdited || 0
      report.value.postsCreated = saved.postsCreated || 0
      report.value.ideasGenerated = saved.ideasGenerated || 0
    }
  } catch (err) {
...
defineExpose({
  getMarketingData: () => ({
    ...
    postsCreated: report.value.postsCreated,
    ideasGenerated: report.value.ideasGenerated,
...
<template>
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
    ...
    <Tabs default-value="metrics" class="w-full">
      <TabsList class="bg-muted/30 p-1.5 rounded-[1.5rem] w-full justify-start gap-2 h-14 mb-8 border border-border/50 backdrop-blur-sm">
        <TabsTrigger value="metrics" class="rounded-xl px-4 flex-1 h-11 font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-zinc-800 shadow-sm">Tráfico & Métricas</TabsTrigger>
        <TabsTrigger value="production" class="rounded-xl px-4 flex-1 h-11 font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-zinc-800 shadow-sm">Producción</TabsTrigger>
      </TabsList>
          ...
          <div class="flex items-center justify-between mb-10">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mt-1">Lo cargado acá se adjunta a tu reporte al cerrar el turno</p>
              </div>
            </div>
          </div>

          <!-- Loading / empty states -->
          <div v-if="loadingAccounts" class="flex items-center justify-center py-16 text-muted-foreground gap-3">
            <RefreshCw class="w-5 h-5 animate-spin" />
            <span class="text-xs font-bold uppercase tracking-widest">Cargando cuentas...</span>
          </div>
          <div v-else-if="report.socialActivity.length === 0" class="text-center py-16 text-muted-foreground">
            <p class="text-sm font-bold">No hay cuentas sociales configuradas.</p>
            <p class="text-xs mt-1">Pedile a un admin que cargue las cuentas desde el CRM.</p>
          </div>
      ...
      <TabsContent value="production">
        <Card class="rounded-[3rem] border-none shadow-xl p-10 bg-blue-500/5 backdrop-blur-xl min-h-[400px] flex items-center justify-center">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-10 w-full">
            <div class="space-y-4 text-center">
              ...
              <Input type="number" v-model.number="report.reelsEdited" class="h-24 text-6xl font-black text-center border-none bg-transparent tabular-nums" />
            </div>
            <div class="space-y-4 text-center">
              <p class="text-[10px] font-black uppercase text-emerald-800/60 tracking-widest">Posts Creados</p>
              <Input type="number" v-model.number="report.postsCreated" class="h-24 text-6xl font-black text-center border-none bg-transparent tabular-nums" />
            </div>
            <div class="space-y-4 text-center">
              <p class="text-[10px] font-black uppercase text-amber-800/60 tracking-widest">Ideas Generadas</p>
              <Input type="number" v-model.number="report.ideasGenerated" class="h-24 text-6xl font-black text-center border-none bg-transparent tabular-nums" />
            </div>
          </div>
        </Card>
      </TabsContent>
